...
public class UserController extends BaseController{
    ...
    /**
     *  2.接收数据的方式：请求处理方法的参数列表设置为非pojo类型，
    ...
    public JsonResult<Void> changeInfo(User user,
                                       HttpSession session){
        //user对象四部分的数据：username、phone、email、gender
        //uid数据需要再次封装到user对象中
        Integer uid=getuidFromSession(session);
        String username=getUsernameFromSession(session);
    ...
    /**
     *  MultipartFile接口是SpringMVC提供的一个接口，这个接口
     *  为我们包装了获取文件类型的数据(任何类型的file都可以接收),Springboot
     *  它又整合了SpringMVC，只需要在处理请求的方法参数列表上声明一个参数类型为
     *  MultipartFile的参数，然后Springboot会自动将传递给服务的文件数据赋值给这个
     *  参数
     * 、@RequestParam 表示请求中的参数，将请求中的参数注入到请求处理的某个方法上，如果
    ...
    public JsonResult<String> changeAvatar(HttpSession session,
        ...
        //获取到这个文件名称，UUID工具类来将生成一个新的字符串来作为文件名
        //例如：avatar01.png
       ...
       // DFSAF1-ASSAD-QDQDAS6-ASDA44-ADASD77.png
